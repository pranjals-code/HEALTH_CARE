...
from app.schemas.auth import (
    ...
    RefreshTokenRequest, RoleCreate, RoleResponse, PermissionCreate,
    PermissionResponse
)
from app.services.auth import AuthService
from app.core.dependencies import get_current_user, get_current_user_response, require_role
...
@router.post("/roles", response_model=RoleResponse, dependencies=[Depends(require_role("SUPER_ADMIN"))])
def create_role(
...
@router.get("/roles", response_model=list[RoleResponse], dependencies=[Depends(require_role("SUPER_ADMIN"))])
def list_roles(
    db: Session = Depends(get_db)
):
...
@router.get("/roles/{role_id}", response_model=RoleResponse, dependencies=[Depends(require_role("SUPER_ADMIN"))])
def get_role(
    role_id: UUID,
    db: Session = Depends(get_db)
):
...
@router.post("/permissions", response_model=PermissionResponse, dependencies=[Depends(require_role("SUPER_ADMIN"))])
def create_permission(
    ...
    existing_perm = db.query(Permission).filter(Permission.name == permission_create.name).first()
    if existing_perm:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
@router.get("/permissions", response_model=list[PermissionResponse], dependencies=[Depends(require_role("SUPER_ADMIN"))])
