...
async def register_with_phone(
    ...
    try:
        ...
        otp_service.store_otp(
            key=f"otp:register:{phone}", otp=otp, ttl=300  # 5 minutes
        )
        ...
        send_otp_sms_task.delay(phone_number=format_e164(req.phone_number), otp=otp)
...
async def verify_phone_otp(
    req: VerifyOTPRequest, request: Request, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        is_valid = otp_service.verify_otp(key=f"otp:register:{phone}", otp=req.otp)
...
async def forgot_password(
    ...
    try:
        ...
        # Generate OTP and store in Redis
        otp = otp_service.generate_otp()
        otp_service.store_otp(key=f"otp:reset:{phone}", otp=otp, ttl=300)  # 5 minutes

        # Trigger Celery task for SMS sending (background)
        send_password_reset_sms_task.delay(
            phone_number=format_e164(req.phone_number), otp=otp
        )
...
async def reset_password(
    ...
    try:
        ...
        # Verify OTP from Redis
        is_valid = otp_service.verify_otp(key=f"otp:reset:{phone}", otp=req.otp)
    ...
    try:
        ...
        return {
            ...
            "message": (
                ...
                if redis_health
                else "Redis not available - OTP storage may fail"
            ),
        }
